...
export default function AuthPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (mode === "login") {
      ...
      // After successful signup, redirect to home or dashboard
      router.push("/");
    }
  };
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        <div className="text-center">
          ...
          <h2 className="mt-6 text-3xl font-extrabold text-gray-900">
            {mode === "login" ? "Welcome Back, Daddy! 👑" : "Become a Daddy Today! 🎯"}
          </h2>
          <p className="mt-2 text-sm text-gray-600">
            {mode === "login" 
              ? "Ready to take charge of your shopping empire?" 
              : "Join the elite club of smart shoppers"}
          </p>
          <p className="mt-2 text-sm text-gray-600">
            {mode === "login" ? "Not a Daddy yet? " : "Already part of the family? "}
            <button
              onClick={() => setMode(mode === "login" ? "signup" : "login")}
              className="font-medium text-green-500 hover:text-green-600 transition-colors"
            >
              {mode === "login" ? "Claim your throne" : "Access your kingdom"}
            </button>
          </p>
        </div>
        ...
        <form className="mt-8 space-y-6 bg-white p-8 rounded-2xl shadow-lg" onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            className="w-full bg-gradient-to-r from-green-400 to-green-600 text-white py-3 rounded-lg font-medium hover:from-green-500 hover:to-green-700 transition-all duration-200 shadow-sm hover:shadow-md transform hover:-translate-y-0.5"
          >
            {mode === "login" ? "Enter Your Kingdom 🚀" : "Become a Daddy Now! 👑"}
          </button>
